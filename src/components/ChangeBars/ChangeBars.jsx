...
// et sa dernière année disponibles. À droite (chaud) = hausse, à gauche
// (cyan) = baisse. Tri du plus en hausse au plus en baisse.
...
export default function ChangeBars({ rows = [], unit, labels = {} }) {
  const [hover, setHover] = useState(null);

  const data = useMemo(
    () => [...rows].filter((r) => Number.isFinite(r.delta)).sort((a, b) => b.delta - a.delta),
    [rows],
  );

  const innerW = VW - M.left - M.right;
  const VH = M.top + M.bottom + data.length * (BAR_H + GAP);
  const maxAbs = useMemo(() => d3.max(data, (r) => Math.abs(r.delta)) || 1, [data]);
  const x = useMemo(
    () => d3.scaleLinear().domain([-maxAbs, maxAbs]).range([M.left, M.left + innerW]),
    [maxAbs, innerW],
  );
  const zero = x(0);
      ...
      >
        ...
        {data.map((r, i) => {
          ...
          return (
            ...
            >
              <text className="cb__name" x={M.left - 12} y={cy + BAR_H / 2} dy="0.32em">
                {r.name}
              </text>
        ...
        {/* Légende */}
        <text className="cb__legend cb__legend--up" x={M.left + innerW} y={VH - 6} textAnchor="end">
          {labels.up} ▸
        </text>
        <text className="cb__legend cb__legend--down" x={M.left} y={VH - 6} textAnchor="start">
          ◂ {labels.down}
        </text>
      </svg>
