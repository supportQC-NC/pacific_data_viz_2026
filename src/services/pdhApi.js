...
// ============================================================
// Accès aux données Pacific Data Hub (.Stat / SDMX) depuis le
// navigateur. L'appel live est "best effort" (plafonné à 8 s) :
// s'il échoue, expire, ou renvoie un jeu vide, on bascule sur un
// jeu de DÉMONSTRATION embarqué (codes PICT réels, multi-années).
// → La page rend TOUJOURS l'expérience : plus de spinner infini.
// (Les jeux officiels du concours remplaceront ces données.)
// ============================================================

const BASE = process.env.REACT_APP_PDH_BASE || 'https://stats-sdmx-disseminate.pacificdata.org/rest/data';

// Délai au-delà duquel on abandonne l'appel live et on bascule en secours.
const LIVE_TIMEOUT_MS = 8000;

// Registre des jeux officiels (clé SDMX = "dq" des liens .Stat).
export const DATASETS = {
  seaLevel:          { flow: 'SPC,DF_CLIMATE_CHANGE,1.0', key: 'A.SEA_LVL.',     start: 1990 },
  sst:               { flow: 'SPC,DF_CLIMATE_CHANGE,1.0', key: 'A.SST_ANOM.',    start: 1990 },
  emissions:         { flow: 'SPC,DF_CLIMATE_CHANGE,1.0', key: 'A.GHG_EMI_CAPITA.', start: 1990 },
  population:        { flow: 'SPC,DF_NMDI_POP,1.0',       key: 'A..NMDI0002._T._T._T..', start: 1990 },
...
function buildUrl({ flow, key, start }) {
  // Les virgules du flow et les points de la clé sont valides dans le chemin.
  const qs = new URLSearchParams({ format: 'csv' });
  if (start) qs.set('startPeriod', String(start));
  return `${BASE}/${flow}/${key}?${qs.toString()}`;
}

// Parsing SDMX-CSV robuste : détecte le séparateur (',' ou ';') et la décimale.
function parseSdmxCsv(text) {
  ...
  const decimalComma = delim === ';'; // colonnes en ';' → décimale en ','
...
// Normalisation : { years, byArea, areas, range, firstYear, lastYear }.
...
// ============================================================
// Données de DÉMONSTRATION (secours) — ordres de grandeur réalistes.
// ============================================================

// Émissions par habitant (t CO₂e/hab.) — base par territoire.
// Outliers connus : Nouvelle-Calédonie (nickel + charbon), Palaos, Guam.
...
const EMISSIONS_YEARS = [2010, 2013, 2016, 2019, 2021];

function buildEmissionsFallback() {
  const rows = [];
  const last = EMISSIONS_YEARS.length - 1;
  EMISSIONS_YEARS.forEach((year, i) => {
    const t = last === 0 ? 1 : i / last;          // 0 → 1
    const drift = 0.88 + 0.24 * t;                // trajectoire douce (~±12 %)
...
// Niveau de la mer (mm cumulés depuis 1993, ~4,1 mm/an) — léger offset par lieu.
...
function buildSeaLevelFallback() {
  ...
  SEALEVEL_YEARS.forEach((year) => {
    ...
    codes.forEach((geo) => {
      rows.push({ geo, year, value: +(rise + (geo.charCodeAt(0) % 5)).toFixed(1) });
    });
  });
  return rows;
}
...
export async function fetchDataset(id, { signal } = {}) {
  ...
  // Plafond strict : on ne reste jamais bloqué "en chargement".
  ...
  try {
    ...
    if (live.years.length > 0) return live;        // données live exploitables
    const fb = fallbackResult(id);                 // live vide → secours si dispo
    return fb || live;
  } catch (err) {
    const fb = fallbackResult(id);                 // live KO (timeout/CORS/HTTP)
    if (fb) return fb;                             // → on sert le secours
    throw err;                                     // pas de secours → échec réel
  } finally {
    clearTimeout(timer);
